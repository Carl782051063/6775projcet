...
unsigned int reverse_bits(unsigned int input) { int i, rev = 0;
    for (i = 0; i < M; i++) {
        rev = (rev << 1) | (input & 1); input = input >> 1;
    }
    return rev;
}

void bit_reverse(DTYPE X R[SIZE], DTYPE X I[SIZE]) { unsigned int reversed;
    unsigned int i;
    DTYPE temp;
    for (i = 0; i < SIZE; i++) {
        reversed = reverse bits(i); // Find the bit reversed index if (i <= reversed) {
// Swap the real values
        temp = X R[i];
        X R[i] = X R[reversed]; X R[reversed] = temp;
// Swap the imaginary values
        temp = X I[i];
        X I[i] = X I[reversed]; X I[reversed] = temp;
    } }
}
